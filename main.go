...
func main() {
	...
	for fmt.Scan(&page.url); page.url != "exit"; fmt.Scan(&page.url) {
		...
		if strings.HasPrefix(page.url, "http://") || strings.HasPrefix(page.url, "https://") {
			...
			log.Info("Parsing URL: %s", page.url)

			res, err := http.Get(page.url)
			if err != nil {
				log.Error("GET query error: %s", err)
			...
			if err != nil {
				log.Error("Response body reading error: %s", err)
				fmt.Println("Response body reading error")
				continue
			}
			err = res.Body.Close()
			if err != nil {
				log.Error("Body closing error: %s", err)
			...
			err = page.save()
			if err != nil {
				log.Error("Error saving HTML file: %s", err)
			...
			err = page.saveMap()
			if err != nil {
				log.Error("Error saving MAP file: %s", err)
			...
			log.Error("Entered wrong URL - %s", page.url)
